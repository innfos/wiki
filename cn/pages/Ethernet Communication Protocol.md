...
执行器开机以后默认为电流模式，如果不是电流模式，请发送激活电流模式指令:</br> EE 06 07 00 01 01 7E 80 ED</br> 其中07代表激活执行器模式指令，00 01代表数据长度，01为电流模式（参见[模式表](#模式表 "wikilink")），7E 80为[CRC校验码](#CRC校验码计算方法 "wikilink"),激活成功后会收到返回：</br> EE 06 07 00 01 01 ED</br> 这时可以发送电流指令，例如：</br> EE 06 08 00 04 00 07 C1 F0 E0 31 ED</br> 其中08为设置电流值的指令，00 07 C1 F0 为设置的电流值，该值是设置目标电流为0.5A,通过公式：目标电流/满量程电流*（2^24）计算取整获得的IQ24值，关于IQ24的介绍请参考[ IQmath简介](#IQmath简介 "wikilink")，关于满量程请参考[型号表](#型号表 "wikilink")。

#### 速度控制

发送激活梯形速度模式指令:</br> EE 06 07 00 01 07 FE 82 ED</br> 数据07为梯形速度模式（参见[模式表](#模式表 "wikilink")），FE 82为[CRC校验码](#CRC校验码计算方法 "wikilink")，激活成功后会收到返回：</br> EE 06 07 00 01 01 ED</br> 这时可以发送速度指令：</br> EE 06 09 00 04 00 08 88 88 E7 80 ED</br> 其中09位设置速度值的指令，00 08 88 88位设置的速度值，该值是设置目标速度200RPM,通过公式：目标速度/6000*（2^24）计算取整获得的IQ24值，关于IQ24的介绍请参考[ IQmath简介](#IQmath简介 "wikilink")。
