...
* 电流环设置电流值范围为_IQ(-1.0)～_IQ(1.0)之间，电流实际值为IQ值乘以满量程，例：（QDD6010型号执行器满量程电流值为33A ，_IQ(0.5) 实际电流值则为0.5x33A=16.5A，见各型号SCA参数表）
